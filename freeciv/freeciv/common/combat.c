                                     ...
                                     const struct tile *tgt_tile)
{
  ...
  unit_list_iterate(tgt_tile->units, punit) {
    ...
    if (punit) {
      /* FIXME: Name of a unit should affect its rules, much less *translation*
       *        of the name, i.e., having different rules depending on the language
       *        of the user. */
      if (strcmp(utype_name_translation(unit_type_get(punit)), "Siege Ram") == 0) {
        /* skip to next unit in the list: Siege Rams don't engage in diplomatic combat. */
        continue;
      }
    }
