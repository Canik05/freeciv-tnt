...
function tech_researched_handler(tech, player, how)
  ...
  id = tech.id
-- Report early Polytheism.
  if id == find.tech_type("Polytheism").id and how == "researched" then
    if game_turn < 15 then
      for c in player:cities_iterate() do 
        if c:has_building(find.building_type("Palace")) then
          ...
          _("<font color=#ffff00>Travelers tell stories of the deeply spiritual %s, who are performing rituals to summon a god near %s! (%i,%i)</font>"),
          player.nation:plural_translation(), c.name, c.tile.x, c.tile.y )

          notify.all( _("A tribe may be summoning a god near %s (%i,%i)"), c.name, c.tile.x, c.tile.y)
        end
      end  
    end
  end
...
"))
  ...
  if turn == 2 then
    notify.event(nil, nil, E.SCRIPT,
_("<b>The gods are real!</b>\n\
All have seen the miracles the gods perform there is no doubt of their power. Build a Shrine to a god for bountiful harvests.\
   (TIP: The Shrine you choose determines the god you can later summon. Check their abilities and choose carefully.)\
"))
  end

  if turn == 12 then
    notify.event(nil, nil, E.SCRIPT,
_("<b>A prophetic warning.</b>\n\
Wise Volur warn the good harvests may soon end as there is anger and turmoil in Asgard.\
"))
  end
end
